define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  return function(config, el, router, d) {
    ...
    if (config.eol)
      if (config.eol.includes(dictGet(d, ["nodeinfo", "hardware", "model"]).toLowerCase()))
        attributeEntry(attributes, "EOL-HINWEIS", function(el) {
          el.classList.add("eol")
          var a = document.createElement("a")
          a.href = config.eol_url
          a.textContent = config.eol_text
          el.appendChild(a)
        })

    if (config.deprecated)
      if (config.deprecated.includes(dictGet(d, ["nodeinfo", "hardware", "model"]).toLowerCase()))
        attributeEntry(attributes, "Warnung", function(el) {
          el.classList.add("deprecated")
          var a = document.createElement("a")
